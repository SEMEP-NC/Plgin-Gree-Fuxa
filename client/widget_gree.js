...
async function exportGreeJSON(intUnits,extUnits){
    const resp=await fetch("/plugins/gree/export",{method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({intUnits,extUnits})});
    if(!resp.ok) throw new Error((await resp.json()).error||"Erreur export Gree");
    const blob=await resp.blob();
    const url=window.URL.createObjectURL(blob);
    const a=document.createElement("a");
    a.href=url; a.download="gree_fuxa.json"; document.body.appendChild(a); a.click(); a.remove();
...
function exportFuxaJSON(intUnits,extUnits){
    ...
    const rows=allUnits.map(u=>({cells:[
    ...
    const viewJson={
        ...
        items:{[tableId]:{id:tableId,type:"svg-ext-own_ctrl-table",name:"Equipements",property:{id:null,type:"data",options:{paginator:{show:false},filter:{show:false},daterange:{show:false},realtime:false,lastRange:"last1h",gridColor:"#E0E0E0",header:{show:true,height:30,fontSize:12,background:"#F0F0F0",color:"#757575"},row:{height:30,fontSize:10,background:"#F9F9F9",color:"#000000"},selection:{background:"#3059AF",color:"#FFFFFF",fontBold:true},columns:[{id:`c_${crypto.randomUUID()}`,label:"Equipements",type:"label",align:"center",width:100},{id:`c_${crypto.randomUUID()}`,label:"Etat",type:"label",align:"center",width:80},{id:`c_${crypto.randomUUID()}`,label:"Consigne",type:"label",align:"center",width:100},{id:`c_${crypto.randomUUID()}`,label:"Temp. Ambiante",type:"label",align:"center",width:100},{id:`c_${crypto.randomUUID()}`,label:"Ventilation",type:"label",align:"center",width:100},{id:`c_${crypto.randomUUID()}`,label:"Défaut",type:"label",align:"center",width:20}],rows:rows}}},variables:{},svgcontent:""};

    const blob=new Blob([JSON.stringify(viewJson,null,2)],{type:"application/json"});
    const url=window.URL.createObjectURL(blob);
    const a=document.createElement("a"); a.href=url; a.download="fuxa_view.json"; document.body.appendChild(a); a.click(); a.remove();
    window.URL.revokeObjectURL(url);
}
